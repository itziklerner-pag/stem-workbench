...
|---|---|---|---|---|
| `void-canary` | `tools/suites/void-canary.mjs` | — | 48 | the runner's own VOID rule, the steps table against this document, the coverage-drift instrument that names an assertion which stopped running, and every mutation battery — bash and JS alike — carrying a restore-on-signal guard and a sentinel |
